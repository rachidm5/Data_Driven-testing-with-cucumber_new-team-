...
public class HomePage extends WebAPI {
    ...
    public void user_clicks_on_Selectable_a_list_of_items_should_appear_Test() {
        HomeP_WebElement.selectable_link.click();
        JavascriptExecutor js = (JavascriptExecutor)driver;
        js.executeScript("arguments[0].scrollIntoView();", HomeP_WebElement.selectable_link);
        driver.switchTo().frame(0);
//        Item_List.forEach(Elem -> System.out.println(Elem));
        for(WebElement element : HomeP_WebElement.Item_List){
            element.isDisplayed();
    ...
    public void user_clicks_on_Sortable_a_list_of_items_should_appear_Test() {
        HomeP_WebElement.Sortable_link.click();
        JavascriptExecutor js1 = (JavascriptExecutor)driver;
        js1.executeScript("arguments[0].scrollIntoView();", HomeP_WebElement.Resizable_link);
        driver.switchTo().frame(0);
//        Sortable_Item_List.forEach( elem -> System.out.println(elem.getText()));
        for(WebElement elem: HomeP_WebElement.Sortable_Item_List){
    ...
    public void user_clicks_on_Resizable_link_a_small_window_with_the_title_should_appear_Test(String Expected_Text) throws AWTException, InterruptedException {
        ...
        Actions ac = new Actions(driver);
        HomeP_WebElement.Resizable_link.click();
        ac.moveToElement(HomeP_WebElement.Dialog_link).build().perform();
//        Thread.sleep(5000);
        driver.switchTo().frame(0);
        String Actual_Title= HomeP_WebElement.Resizable_Text.getText();
    ...
    public  void user_should_be_able_to_Resize_the_small_window_to_a_desired_Test(String size) throws InterruptedException, IOException {
        HomeP_WebElement.Resizable_Icon.getLocation();
        System.out.println(HomeP_WebElement.Resizable_Icon.getLocation());
        takeScreenshot(driver);
        Actions ac= new Actions(driver);
        ac.clickAndHold(HomeP_WebElement.Resizable_Icon).moveByOffset(200,200).build().perform();
        takeScreenshot(driver);
//        Thread.sleep(5000);
    }
}
